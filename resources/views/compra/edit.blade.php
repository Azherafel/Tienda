...
@section('form')
    ...
    <div class="shadow p-4 mb-5 bg-body-tertiary rounded">
        <form action="{{ route('compras.update', ['compra' => $compra->id]) }}" method="POST" id="frm-compra">
            @method('PUT')
            @csrf
            <div class="mb-4 fs-5">
                ...
                <div class="d-flex gap-2">
                    <select name="cliente_id" class="form-select" id="cliente_id">
                        ...
                        @foreach ($clientes as $cliente)
                            <option value="{{ $cliente->id }}" {{ old('cliente_id', $compra->cliente_id) == $cliente->id ? 'selected' : '' }}>{{ $cliente->nombre }} {{ $cliente->apellidoP }} {{ $cliente->apellidoM }}</option>
                        @endforeach
                    </select>
            ...
            <div class="mb-4 fs-5">
                ...
                <select name="estado" class="form-select" id="estado">
                    @foreach (['pendiente', 'pagada', 'cancelada', 'entregada'] as $estadoOpcion)
                        <option value="{{ $estadoOpcion }}" {{ old('estado', $compra->estado) == $estadoOpcion ? 'selected' : '' }}>{{ ucfirst($estadoOpcion) }}</option>
                    @endforeach
                </select>
                @error('estado')
                    <p class="text-danger small mt-1">{{ $message }}</p>
            ...
            <div class="mb-4 fs-5">
                ...
                <input name="metodoPago" type="text" class="form-control" id="metodoPago" placeholder="Efectivo, tarjeta, transferencia..." value="{{ old('metodoPago', $compra->metodoPago) }}">
                @error('metodoPago')
                    <p class="text-danger small mt-1">{{ $message }}</p>
                @enderror
            </div>
    ...
    <script type="module">
        $(document).ready(function () {
            let filaIndex = 0;
            const oldProductos = @json(old('productos', $compra->compraProductos->map(function ($cp) {
                return ['producto_id' => $cp->producto_id, 'cantidad' => $cp->cantidad];
            })));
